...
代理默认监听 `3101`。若上游 `127.0.0.1:58627` 没有 kimi web，它会自动 `spawn` 一个 `kimi web --port 58627 --no-open` 并沿用 `~/.kimi-code/server.token` 里的持久 token。
...
代理做了两类改写，对 kimi 前端版本的敏感度不同：

1. **版本无关的通用前缀改写（HTML/CSS/JS）**。根绝对资源引用用纯字符串替换加前缀。这类引用是结构性写法，不随版本变化，因此无论前端 bundle 怎么换 hash，这部分始终有效。
2. **针对特定 bundle 的 JS patch（`Wke()` origin 推导）**。为了让 API/WS 落到转发子树，代理需要改写 kimi 前端 origin 解析函数里的压缩表达式。这个匹配针对的是某个具体前端构建产出的压缩字符串。

**已验证版本：Kimi Code `0.37.2`** —— server `server_version: 0.37.2`，前端 bundle `index-yKYHPeXU.js` / `index-DBtc5buz.css`。

**如果后续 kimi 版本换了 bundle：** patch 匹配失败时，代理启动会在日志里打印 WARN（`origin-resolution pattern was not found`），**绝不会静默带出一个坏 UI**。此时：
...
2. **正式修复**：更新 `proxy.js` 里的 `WKE_QUOTED` / `WKE_PATCH_RE` 常量以匹配新的压缩表达式，并在 `tests/fixtures/` 下补一份对应 fixture，让回归被测试覆盖。
