...
2. **Route-constant value replacement (JS).** The three route constants are matched as complete quoted values — semantic route paths, far more stable across versions than minified identifiers (identical in 0.37.2 and 0.39.1). If kimi ever renames its routes, deep-link restore would degrade, but the injected history guard still keeps navigation inside the base subtree.
...
**Verified against: Kimi Code `0.37.2` and `0.39.1`** (frontend bundles `index-yKYHPeXU.js` / `index-ClWTW3HX.js`). The `--dangerous-bypass-auth` option needs kimi >= 0.39; on older versions the proxy detects the failed spawn and retries without the flag (token seeding still applies).
